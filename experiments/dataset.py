...
from experiments.pfedhn_seg.monai_datasets import get_datasets
...
#     val_set, test_set = torch.utils.data.random_split(val_test_set, [val_size, test_size])
#     print(f"{type(train_set)} | train_set: {len(train_set)}, val_set: {len(val_set)}, test_set: {len(test_set)}")
#
...
def gen_loaders(data_names, data_path, batch_size):
    ...
    :return: train/val/test loaders of each client, list of pytorch dataloaders
    """
    train_loader_params = {"batch_size": batch_size, "shuffle": True, "pin_memory": True, "num_workers": 0}
    val_test_loader_params = {"batch_size": 1, "shuffle": False, "pin_memory": True, "num_workers": 0}
    dataloaders = []
    # list of tuples, of form (train,val,test) for each data name
    datasets_tuples = list(map(lambda data_name: get_datasets(data_name, data_path), data_names))
    for i in range(len(datasets_tuples[0])):
        datasets = list(map(lambda dataset_tuple: dataset_tuple[i], datasets_tuples))
        # create dataloaders from subsets
        if i == 0:
            dataloaders.append(list(map(lambda x: DataLoader(x, **train_loader_params), datasets)))
        else:
            dataloaders.append(list(map(lambda x: DataLoader(x, **val_test_loader_params), datasets)))
